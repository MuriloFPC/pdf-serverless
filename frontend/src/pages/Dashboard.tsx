...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    fetchJobs();
    // Poll for status updates every 10 seconds if there are pending jobs
    const interval = setInterval(() => {
      const hasPending = jobs.some(j => j.status === 'awaiting_files' || j.status === 'processing' || j.status === 'pending');
      if (hasPending) {
        fetchJobs();
      }
    }, 10000);
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      ...
      case 'awaiting_files': return 'Aguardando Arquivos';
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'unprotect': return 'Desproteger';
      default: return type;
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-dark-400 mt-1">Acompanhe o status dos seus arquivos</p>
        </div>
      ...
      {loading && jobs.length === 0 ? (
        ...
        <div className="bg-dark-900 border border-dark-800 rounded-2xl p-12 text-center">
          ...
          <p className="text-dark-400 mb-8 max-w-sm mx-auto">
            Você ainda não criou nenhum processo de PDF. Comece criando um novo job!
          </p>
          <Link
            to="/new-job"
