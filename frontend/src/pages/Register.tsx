    ...
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center px-4">
      <div className="w-full max-w-md space-y-8 bg-dark-900 p-8 rounded-2xl border border-dark-800 shadow-xl">
        <div className="text-center">
          ...
          <h2 className="text-3xl font-bold">{t('register_page.title')}</h2>
          <p className="mt-2 text-dark-400">{t('register_page.subtitle')}</p>
        </div>

        {success ? (
          <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 p-6 rounded-xl flex flex-col items-center gap-3 text-center">
            <CheckCircle size={48} className="mb-2" />
            <h3 className="font-bold text-lg">{t('common.success')}</h3>
          ...
          <>
            <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
              ...
              >
                {loading ? <Loader2 className="animate-spin" size={20} /> : t('register_page.cta')}
              </button>
            </form>

            <p className="text-center text-sm text-dark-400">
              {t('register_page.have_account')}{' '}
              <Link to="/login" className="text-blue-500 hover:text-blue-400 font-medium inline-flex items-center gap-1 transition-colors">
                {t('register_page.login_link')} <LogIn size={14} />
              </Link>
            </p>
          </>
        )}
